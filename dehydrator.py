...
# --- 脱水提示词：指导廉价 LLM 压缩信息 ---
DEHYDRATE_PROMPT = """你是一个信息整理专家。请将以下内容整理为保真摘要，以保留为先、删减为辅。

整理规则：
1. 保留所有核心事实、具体细节和有画面感的描述，只去除真正的重复啰嗦
2. 保留最新的情绪状态和态度，情绪表达尽量用原句
3. 保留所有待办/未完成事项
4. 关键数字、日期、名称、原话引用必须保留
5. 目标压缩率 30%~50%，宁可长一点也不要丢细节和语气
...
2. 为每个条目自动分析元数据
3. 只去除真正无意义的重复；细节、原话、情绪表达、有画面感的描述都要保留进 content
4. 同一主题的零散信息应合并为一个条目，但合并时不丢任何一方的细节
5. 如果有待办事项，单独提取为一个条目
6. content 不是概括，是"整理后的原文"——宁可长，不要总结成干巴巴的几个字
...
# --- 合并提示词：指导 LLM 揉合新旧记忆 ---
MERGE_PROMPT = """你是一个信息合并专家。请将旧记忆与新内容合并为一份统一的记录。

合并规则：
1. 新内容与旧记忆冲突时，以新内容为准
2. 只去除完全重复的信息
3. 保留双方所有事实、细节、原话和情绪表达，不要为了缩短而概括
4. 总长度尽量不超过旧记忆与新内容之和的 150%
